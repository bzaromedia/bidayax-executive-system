...
| 11B-PRR-051 | CRITICAL | Post-merge closure Advisor found the PostgreSQL verifier could target an unintended remote or production-like database before rollback. | Resolved in the corrective branch by exact local disposable database URL policy and pre-connection URL safety self-tests covering non-local hosts, exact database-name enforcement, query-parameter host/port/SSL/service overrides, encoded and duplicated parameters, mixed-case parameters, remote IPv6, and Unix-socket-style host attempts. | Phase 11B |
