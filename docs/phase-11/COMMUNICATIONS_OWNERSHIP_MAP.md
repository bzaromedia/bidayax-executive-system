...
| `telephony_calls` | Legacy preparation table | Existing legacy writer only | None for Phase 11B | Quarantined with migration `0005`; not a Communications write target. |
| `telephony_call_events` | Legacy preparation table | Existing legacy writer only | None for Phase 11B | Quarantined with migration `0005`; Communications audit and lifecycle evidence must use Communications-owned records. |
| `voice_sessions` | Legacy preparation table | Existing legacy writer only | None for Phase 11B | Quarantined with migration `0005`; receptionist-session references must use the Phase 11B Communications boundary. |
| `outbound_call_requests` | Legacy preparation table | Existing legacy writer only | None for Phase 11B | Quarantined with migration `0005`; outbound dispatch remains disabled until later authorization. |
...
| `telephony_call_queues` | Adapter-local queue metadata | Telephony adapter | Store transport queue capability only | Communications routing must use Communications-owned routing policy and may reference queue capability only through tenant-composite constraints. |
...
| `telephony_appointment_requests` | Adapter-local or legacy appointment evidence | Existing legacy writer only until cutover | Surface transport scheduling metadata only | Communications scheduling intent must use Communications-owned records; calendar or orchestration behavior remains outside Phase 11B. |
...
| `telephony_voice_profiles` | Adapter-local voice capability metadata | Telephony adapter | Store provider voice mapping metadata only | Communications may not use this table to activate voice providers or Phase 11E behavior in Phase 11B. |
...
| `telephony_voicemails` | Adapter-local voicemail metadata | Telephony adapter | Store transport voicemail references only | Communications message or receptionist outcomes must use Communications-owned evidence; raw audio/transcript storage remains prohibited. |
...
| `telephony_command_idempotency_keys` | Adapter-local command idempotency metadata | Telephony adapter | Deduplicate adapter transport commands only | Communications command idempotency must use Communications-owned idempotency records; the two boundaries must not share keys or write paths. |
