...
| Trust evidence sanitized links | `communication_trust_evidence_references`; allowlisted trust evidence fields; envelope-required payload projection; migration `0019` database-owned post-0019 `recorded_at`, Trust event envelope binding, historical signing-key validity checks, and legacy-row revalidation; Communications Trust runtime domains in `packages/trust/src/domains.ts` | Domain tests; Trust package tests; PostgreSQL sensitive-field, non-allowlisted-field, missing-envelope, missing Trust event envelope link, wrong-key-purpose, payload-mismatch, event/domain mismatch rejection, 0018-to-0019 missing-envelope-link rejection, retired/retiring/revoked-after-recorded/compromised-after-recorded historical key acceptance, compromised-before-recorded rejection, and post-migration revoked/compromised backdated-reference rejection | Implementation merged by PR #15; post-merge guard remediation active |
