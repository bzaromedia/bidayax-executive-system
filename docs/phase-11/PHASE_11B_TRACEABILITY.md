...
| Durable idempotency and retries | `communication_command_idempotency_keys`; repository contracts `reserveCommandIdempotency` and `completeCommandIdempotency`; migration `0019` database-owned command creation timestamp, migration-owned reserved-command invalidation, preserved terminal command outcomes, and strengthened legacy-invalidation constraints | Domain tests; PostgreSQL duplicate-key rejection, terminal-insert rejection, immutable authorization fields, backdated expired session/grant rejection, controlled reserved-to-terminal result update, preserved pre-0019 terminal command evidence, forged legacy invalidation rejection, direct invalidated-insert rejection, and 0018-to-0019 reserved-command invalidation requiring a new command key and fresh authorization | Implementation merged by PR #15; post-merge guard remediation active |
| Canonical lifecycle transitions | `communication_lifecycle_transitions`; transition trigger in migration `0018`; migration `0019` null-safe required-key authorization guard and locked legacy-row revalidation | PostgreSQL verifier checks valid transition, invalid transition, terminal re-entry rejection, backdated-transition rejection, forged aggregate update rejection, append-only behavior, missing/null authorization metadata rejection, and 0018-to-0019 fail-closed legacy lifecycle evidence | Implementation merged by PR #15; post-merge guard remediation active |
| Consent policy and receipt separation | `communication_consent_policies`; `communication_consent_receipts`; consent trust-evidence FK and migration `0019` required-key/null-safe trigger guards plus current Trust-key authority checks and policy-before-Trust lock ordering | PostgreSQL verifier checks active, missing, nonexistent, mismatched, omitted-identity, omitted-timing, mismatched-timing, expired, revoked, invalid-chronology, revoked-key, compromised-key, ambiguous consent evidence, malformed lock tuples, subject-mismatched references, legacy non-string and JSON-null tuple rejection, dispatch outcomes, consent-reference/receipt versus dispatch contention without deadlock, and the atomic consent envelope versus queued dispatch lock interleaving | Implementation merged by PR #15; post-merge guard remediation active |
| Suppression enforcement | `communication_suppressions`; dispatch policy trigger; controlled release trigger; migration `0019` null-safe required-key release-audit guard and locked legacy-row revalidation | PostgreSQL verifier checks active suppression denial, missing release-audit rejection, controlled active-to-released transition, double-release rejection, post-release dispatch eligibility, missing/null release metadata rejection, and 0018-to-0019 fail-closed legacy suppression release evidence | Implementation merged by PR #15; post-merge guard remediation active |
...
| Trust evidence sanitized links | `communication_trust_evidence_references`; allowlisted trust evidence fields; envelope-required payload projection; migration `0019` post-lock database-owned post-0019 `recorded_at`, FK-compatible row-locked Trust-key lifecycle validation, string-typed Trust event envelope binding, historical signing-key validity checks, current authorization-key checks, and legacy-row revalidation; Communications Trust runtime domains in `packages/trust/src/domains.ts` | Domain tests; Trust package tests; PostgreSQL sensitive-field, non-allowlisted-field, missing-envelope, missing Trust event envelope link, non-string Trust event envelope identifier rejection, wrong-key-purpose, payload-mismatch, event/domain mismatch rejection, 0018-to-0019 missing-envelope-link and non-string envelope identifier rejection, retired/retiring/revoked-after-recorded/compromised-after-recorded historical key acceptance, compromised-before-recorded rejection, post-migration revoked/compromised backdated-reference rejection, two-connection Trust-key revocation/compromise contention rejection with `pg_blocking_pids`, reference-first compromise/revocation blocking evidence, consent/dispatch revoked-key and compromised-key authority rejection, and reverse-order pre-lifecycle acceptance | Implementation merged by PR #15; post-merge guard remediation active |
...
| Rollback or corrective roll-forward safety | Forward-only migrations `0018` and `0019`; rollback is disabling new writers/readers or corrective roll-forward without deleting evidence; pre-0019 reserved command rows are failed for fresh reauthorization while existing terminal command outcomes are preserved | PostgreSQL verifier applies migrations through `0018`, seeds representative valid and invalid 0018-era metadata, lifecycle, suppression, consent, Trust, and command evidence, applies `0019`, verifies valid metadata, lifecycle, suppression, consent, terminal command, and historical Trust-key rows survive, verifies reserved legacy commands are failed as reauthorization-required evidence, verifies invalid legacy rows fail closed, verifies forged legacy invalidation and direct invalidated insert are rejected, verifies concurrent Trust-key lifecycle changes serialize against Trust-reference creation in both lifecycle-first and reference-first orderings, verifies consent-reference/receipt and dispatch use a consistent policy-before-Trust lock order without deadlock including the envelope-FK interleaving, verifies current authorization denies compromised/revoked Trust evidence, verifies constraints are validated, applies the full migration chain to the exact local disposable database name, and rolls back verification fixture data | Post-merge corrective roll-forward active |
...
| Finding | Severity | Remediation evidence | Validation evidence |
| --- | --- | --- | --- |
| Existing 0018 authorization, consent, Trust, and command timestamp evidence was not revalidated by `0019`. | CRITICAL | Migration `0019` now performs locked legacy-row scans under one lock boundary, invalidates pre-0019 reserved command idempotency rows as failed evidence requiring new command keys and fresh authorization, preserves already-terminal command outcomes, validates existing Trust evidence references against envelope, historical key validity, verification, signed payload, and Trust event linkage requirements, validates existing consent receipts against required identity, timing, chronology, and policy-version evidence, and validates lifecycle/suppression authorization metadata with null-safe required-key checks. | PostgreSQL verifier seeds valid legacy metadata, lifecycle, suppression, consent, Trust, and command evidence plus invalid consent, lifecycle, suppression, Trust-event, compromised-key, and hash rows before applying `0019`; valid legacy rows and terminal command outcomes survive, reserved legacy commands are preserved as failed reauthorization-required evidence, and invalid legacy states fail closed during upgrade. |
| The PostgreSQL upgrade gate did not cover legacy authorization, command timestamp, consent, or Trust evidence paths. | HIGH | `scripts/verify-communications-data-model-postgres.ts` now runs separate 0018-to-0019 scenarios for reserved-command invalidation, terminal-command preservation, missing/null lifecycle authorization evidence, missing/null suppression release evidence, missing consent evidence fields, missing Trust event envelope linkage, invalid compromised-key timing, invalid hash metadata, valid consent survival, post-upgrade invalid writes, forged legacy invalidation, direct invalidated insert rejection, concurrent old-writer rejection, and metadata-constraint validation. | `pnpm verify:communications-data-model:postgres` reports the expanded 0018-to-0019 evidence string and passes. |
...
Final-head Advisor review on 2026-08-01 at
`1810cb31bb1ce95e9b1d2ff9acfa3648b28fb2eb` returned
`ADVISOR_REJECTED_FOR_MERGE` because three merge-blocking correctness gaps
remained in the corrective package.

| Finding | Severity | Remediation evidence | Validation evidence |
| --- | --- | --- | --- |
| Lifecycle and suppression authorization evidence still failed open under SQL NULL semantics. | CRITICAL | Migration `0019` now replaces the lifecycle and suppression evidence triggers with required-key and `IS DISTINCT FROM` comparisons and performs locked legacy-row scans before accepting the upgrade. | PostgreSQL verifier rejects omitted and JSON-null lifecycle authorization metadata, rejects omitted and JSON-null suppression release metadata, and fails closed on invalid 0018-era lifecycle and suppression evidence during the 0018-to-0019 upgrade path. |
| Legacy-command migration invalidation was client-forgeable through a custom GUC and direct invalidated inserts. | HIGH | Migration `0019` removes the caller-writable invalidation bypass, performs migration-owned reserved-command invalidation by dropping and recreating the result-update trigger under lock, strengthens the invalidation constraint, and preserves already-terminal command outcomes. | PostgreSQL verifier rejects forged legacy invalidation settings, rejects direct invalidated inserts, verifies reserved legacy commands require fresh reauthorization, and verifies existing terminal command outcomes remain intact. |
| Trust-key rotation could create an unrecoverable migration dead end because only currently active keys were accepted. | HIGH | Migration `0019` now distinguishes historical signing validity from current dispatch eligibility by accepting retiring, retired, revoked-after-recorded, and compromised-after-recorded key evidence while rejecting pending, revoked-before-recorded, and compromised-before-recorded evidence. | PostgreSQL verifier accepts historically valid retired, retiring, revoked, and compromised key states after the evidence was recorded and rejects compromised-before-recorded legacy Trust evidence during upgrade. |
| PRR and traceability were stale after the rejected final-head review. | MEDIUM | The PRR and traceability matrix now document the `1810cb3` rejection, the null-safe lifecycle/suppression remediation, migration-owned command invalidation, historical Trust-key rules, and the expanded executable evidence. | Documentation is included in the corrective diff and must be revalidated by repository gates and fresh Advisor review. |

Final-head Advisor review on 2026-08-01 at
`2cb7fcab2082102d5d90966808623a70261cc142` returned
`ADVISOR_REJECTED_FOR_MERGE` because post-0019 Trust-reference recording time
could still be caller-forged after key compromise or revocation.

| Finding | Severity | Remediation evidence | Validation evidence |
| --- | --- | --- | --- |
| Post-compromise Trust-reference creation remained caller-forgeable through supplied `recorded_at`. | CRITICAL | Migration `0019` now assigns `communication_trust_evidence_references.recorded_at` from database wall-clock time on insert before Trust key revocation/compromise cutoff checks. Existing 0018-era rows remain evaluated by the locked legacy scan so legitimate historical evidence can survive upgrade. | PostgreSQL verifier attempts post-0019 Trust-reference inserts after key compromise and revocation with caller-backdated `recorded_at`; both fail closed while existing 0018-era historical Trust-key evidence still survives the 0018-to-0019 upgrade path. |
| PRR and traceability overstated Trust-key remediation after the rejected final-head review. | HIGH | The PRR and traceability matrix now document the `2cb7fca` rejection, the database-owned post-0019 Trust-reference timestamp rule, and the revoked/compromised backdating regression coverage. | Documentation is included in the corrective diff and must be revalidated by repository gates and fresh Advisor review. |
| Evidence-document dates were stale. | LOW | PRR, traceability, and current-phase status dates are updated to the remediation date. | Documentation inspection and governance validation. |

Final-head Advisor review on 2026-08-01 at
`a7dddc86a1afee6fd57f69042dafaebb45306f6c` returned
`ADVISOR_REJECTED_FOR_MERGE` because Trust-reference validation did not
serialize with concurrent Trust-key revocation or compromise.

| Finding | Severity | Remediation evidence | Validation evidence |
| --- | --- | --- | --- |
| Trust-reference creation could race an uncommitted Trust-key compromise or revocation because the trigger read `trust_keys` without a conflicting row lock. | CRITICAL | Migration `0019` now locks the cited Trust key row before evaluating revocation or compromise cutoffs for post-0019 Trust-reference creation. The insert waits for concurrent lifecycle updates and then rechecks the committed lifecycle state before accepting evidence. | `pnpm verify:communications-data-model:postgres` runs committed-fixture two-connection tests for uncommitted key compromise and uncommitted key revocation; each contender insert must block and then reject after the lifecycle transaction commits. |
| PRR and traceability overstated Trust concurrency coverage after the rejected final-head review. | HIGH | The PRR and traceability matrix now document the `a7dddc8` rejection, the row-locked Trust-key lifecycle rule, and the additional two-connection regression coverage. | `pnpm verify:communications-data-model:postgres` also verifies the reverse ordering: a Trust reference recorded before a later key compromise remains accepted. |

Read-only database and security sidecar reviews on 2026-08-01 rejected the
working tree because the Trust-key lock existed but the insert timestamp was
still captured before lock acquisition, and because compromised or revoked keys
remained authorization-eligible for consent and dispatch.

| Finding | Severity | Remediation evidence | Validation evidence |
| --- | --- | --- | --- |
| `recorded_at` was captured before the Trust-key lock, so a lifecycle transaction that locked first could still commit a cutoff after the stale insert timestamp and have the insert accepted. | CRITICAL | Migration `0019` now assigns post-0019 Trust-reference `recorded_at` only after acquiring the cited Trust-key row lock and evaluates envelope expiry, key lifecycle cutoffs, and verification receipts against that post-lock timestamp. The lock mode is FK-compatible so envelope creation does not deadlock with policy checks while real Trust-key lifecycle updates still serialize. | `pnpm verify:communications-data-model:postgres` tests lifecycle-first compromise and revocation where the lifecycle transaction owns the key lock before insert timestamping; `pg_blocking_pids` must identify the lifecycle transaction as the blocker and the insert must reject after commit. |
| Compromised or revoked Trust-key evidence remained usable for consent and dispatch authorization, diverging from the Trust runtime verification policy. | HIGH | Migration `0019` adds `communication_trust_reference_has_authoritative_key_v1`, locks the signing key row, requires current non-revoked and non-compromised Trust-key authority for consent receipt validation, and adds a queued dispatch guard for the cited consent evidence. | PostgreSQL verifier rejects new consent receipt creation and queued dispatch when the cited evidence key is compromised or revoked. |
| Concurrency tests inferred blocking from elapsed time and did not prove both serialization directions. | HIGH | The verifier now checks `pg_blocking_pids` for the exact blocker backend, tests lifecycle-first compromise and revocation, and tests reference-first compromise and revocation where the lifecycle update waits behind the Trust-reference insert. | `pnpm verify:communications-data-model:postgres` passes with the expanded contention and reverse-order evidence. |
| Consent Trust-reference/receipt insertion and queued dispatch used inconsistent policy/Trust lock ordering, creating a deadlock path under concurrent consent and dispatch authorization, including malformed and subject-mismatched consent evidence. | MEDIUM | Migration `0019` now makes consent Trust-reference and receipt validation acquire the policy-subject advisory lock before Trust-key authority validation, rejects malformed consent lock tuples before Trust-key locking, and rejects subject mismatches before receipt policy locking. | `pnpm verify:communications-data-model:postgres` exercises actual consent-reference, consent-receipt, and dispatch insert triggers with two connections, verifies the expected blocker with `pg_blocking_pids`, and requires malformed or mismatched evidence to fail without `40P01`. |
| Legacy 0018 consent evidence with non-string tuple values could authorize receipts through PostgreSQL `->>` text coercion. | MEDIUM | Migration `0019` now requires string-typed consent tuple fields during live receipt validation and legacy revalidation before policy or Trust authority is accepted. | `pnpm verify:communications-data-model:postgres` includes 0018-to-0019 fixtures for numeric, object, and array participant tuple values plus JSON-null values for 0018-persistable consent tuple fields, and requires migration 0019 to reject them. |
| Trust-event envelope binding could authorize live or legacy Trust references through PostgreSQL `->>` text coercion of non-string `details.envelopeId`. | MEDIUM | Migration `0019` now requires string-typed Trust-event envelope identifiers during live Trust-reference validation and legacy revalidation. | `pnpm verify:communications-data-model:postgres` includes live fixtures for numeric, JSON null, object, and array Trust-event envelope identifiers and 0018-to-0019 fixtures for numeric, object, and array coercion-compatible legacy identifiers; all must reject. |

Pre-commit Advisor review on 2026-08-05 returned `ADVISOR_REJECTED`
because one High lock-order defect and one Medium evidence-parity defect
remained in the corrective package.

| Finding | Severity | Remediation evidence | Validation evidence |
| --- | --- | --- | --- |
| Atomic consent evidence creation could deadlock with queued dispatch because the consent transaction's Trust-key foreign-key `KEY SHARE` lock conflicted with dispatch's Trust-key `FOR UPDATE` lock while dispatch held the policy advisory lock. | HIGH | Migration `0019` now uses `FOR NO KEY UPDATE` for Trust-key authority and compatibility reads, preserving conflict with real Trust-key lifecycle updates while remaining compatible with envelope foreign-key locks. | `pnpm verify:communications-data-model:postgres` includes an atomic consent envelope versus dispatch interleaving that verifies the consent Trust-reference insert blocks behind the policy owner without `40P01`, while the existing Trust-key lifecycle contention tests still prove lifecycle updates serialize with Trust-reference creation. |
| Evidence documents claimed legacy JSON-null consent tuple upgrade coverage, but the verifier covered only numeric, object, and array non-string participant tuples. | MEDIUM | The PostgreSQL verifier now seeds 0018-era JSON-null values for the consent evidence tuple fields that can persist under the 0018 trigger before applying migration `0019`; JSON-null `policyVersion` remains covered by the post-0019 live negative because 0018 already rejects it. | `pnpm verify:communications-data-model:postgres` requires migration `0019` to fail closed on those legacy JSON-null consent evidence rows. |
