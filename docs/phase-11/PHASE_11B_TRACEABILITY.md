...
| Rollback or corrective roll-forward safety | Forward-only migrations `0018` and `0019`; rollback is disabling new writers/readers or corrective roll-forward without deleting evidence | PostgreSQL verifier applies migrations through `0018`, seeds representative valid and invalid 0018-era metadata, applies `0019`, verifies valid rows survive, verifies invalid legacy rows fail closed, verifies constraints are validated, applies the full migration chain to the exact local disposable database name, and rolls back verification fixture data | Post-merge corrective roll-forward active |
