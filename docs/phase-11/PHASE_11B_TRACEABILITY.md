...
| Durable idempotency and retries | `communication_command_idempotency_keys`; repository contracts `reserveCommandIdempotency` and `completeCommandIdempotency`; migration `0019` database-owned command creation timestamp | Domain tests; PostgreSQL duplicate-key rejection, terminal-insert rejection, immutable authorization fields, backdated expired session/grant rejection, and controlled reserved-to-terminal result update | Implementation merged by PR #15; post-merge guard remediation active |
...
| Rollback or corrective roll-forward safety | Forward-only migrations `0018` and `0019`; rollback is disabling new writers/readers or corrective roll-forward without deleting evidence | PostgreSQL verifier applies the full migration chain to the exact local disposable database name and rolls back verification fixture data | Post-merge corrective roll-forward active |
| Named PostgreSQL validation gate | `scripts/verify-communications-data-model-postgres.ts`; `package.json` script; `.github/workflows/ci.yml` required step | `pnpm verify:communications-data-model:postgres` | Passed locally on the post-merge corrective working tree with 19 migrations; remote CI pending after push |
| Production Readiness Review | `docs/phase-11/PHASE_11B_PRODUCTION_READINESS_REVIEW.md` | Required final disposition: `PRODUCTION_READINESS_REVIEW_PASSED` before any corrective implementation merge | Passed locally for the repaired corrective working tree; remote CI and Advisor gate pending |
| Repository validation | Workspace scripts and repository gates | `pnpm install --frozen-lockfile`; `pnpm verify`; `pnpm verify:production`; policy verifiers; `git diff --check`; secret-pattern scan | Focused validation, policy checks, and full `pnpm verify` passed locally on the repaired corrective working tree; remote CI pending after push |
| Advisor approval | Independent read-only Advisor review | Required disposition: `ADVISOR_APPROVED_FOR_MERGE` after final local validation and final-head CI | Pending final corrective gate |
...
| The PostgreSQL verifier accepted unsafe database targets through substring database-name matching. | CRITICAL | `scripts/verify-communications-data-model-postgres.ts` now allows only the exact local disposable database name on local hosts and runs URL safety self-tests before connecting or applying migrations, including query-parameter host, port, SSL, service, encoded, duplicated, and mixed-case override attempts plus remote IPv6 and Unix-socket-style host attempts. | `pnpm verify:communications-data-model:postgres` passes against the local disposable target and rejects unsafe URL examples internally. |
| TypeScript metadata validation accepted nested runtime objects and arrays before persistence rejected them. | HIGH | `assertSafeCommunicationMetadata` now rejects non-primitive runtime metadata values. | Domain tests reject nested object and array metadata values. |
