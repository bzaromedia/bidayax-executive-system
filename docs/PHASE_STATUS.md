...
The workspace intentionally excludes placeholder directories without `package.json` files from the active release graph:
...
These are not v1.0 production packages and must not be advertised as active product capability.
